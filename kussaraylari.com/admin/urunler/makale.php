...
if(isset($_POST['btn_delete']))
	{
		...
		$id = $_POST['delete_id'];
		$stmt_select = $DB_con->prepare('SELECT makale_resim FROM blog WHERE makale_id =:bid');
		$stmt_select->execute(array(':bid'=>$id));
		$imgRow=$stmt_select->fetch(PDO::FETCH_ASSOC);
		unlink("../image/blog_images/".$imgRow['makale_resim']);
		
		
		$stmt_delete = $DB_con->prepare('DELETE FROM blog WHERE makale_id =:bid');
		$stmt_delete->bindParam(':bid',$id);
		$stmt_delete->execute();
...
<h1 class="h2">Kayıtlı Makaleler <a class="btn btn-default buton_ekle" href="makale_ekle.php"> <span class="glyphicon glyphicon-plus "></span> Makale Ekle</a></h1>
...
<tr>
    <td class="genislik"><label class="control-label"><?php echo $makale_id; ?>. Makale Başlık</label></td>
    <td><?php echo $row['makale_baslik']; ?></td>
</tr>
<tr>
    <td class="genislik"><label class="control-label">    Başlık id</label></td>
    <td><?php echo $row['baslik_id']; ?></td>
</tr>
<tr>
    <td class="genislik"><label class="control-label">    Makale Tarih</label></td>
	<td><?php echo $row['makale_tarih']; ?></td>
</tr>
<tr>
    <td class="genislik"><label class="control-label" >    Makale İçerik</label></td>
	<td><?php echo $row['makale_icerik']; ?></p></td>
</tr>
<tr>
    <td class="genislik"><label class="control-label">    Makale Resim</label></td>
    <td><p><img src="../image/blog_images/<?php echo $row['makale_resim']; ?>" class="img-rounded" style="background-size:contain;" height="200px" width="200px" /></p>
    </td>
</tr>
<tr>
    <td class="genislik"><label class="control-label">Makale Url</label></td>
    <td><?php echo $row['makale_url']; ?></td>
</tr>
</table>

</form>
<div class="kutu1" style="float: left; padding: 10px;">
<form method="post" action="makale_duzenle.php">
    <tr>
        <td ><input type="hidden" name="edit_id" value="<?php echo $makale_id; ?>" />
...
<form method="post" action="makale.php">
    <tr>
        <td><input type="hidden" name="delete_id" value="<?php echo $makale_id; ?>" />
...
<?php
	...
	{
		?>
		<a class="btn btn-info" href="makale.php" style="background:#60aaeb; float:right; margin:5px;"><span class="glyphicon glyphicon-backward"></span>   Başa Dön</a>
...
if($page != 1) echo '<li><a href="makale?start=1">&lt;&lt; ilk sayfa</a></li>';
if($page != 1) echo '<li><a href="makale?start='.($geri).'">&lt; önceki</a></li>';
 
for($s = $sol_sayfalar; $s <= $sag_sayfalar; $s++) {
    if($page == $s) {
        echo  '<li class="active secili"><a ">'.$s.'</a></li>';
    } else{
      echo  '<li><a href="makale?start='.$s.'" style="cursor:pointer;">'.$s.'</a></li>';
      }
}
